...
import math
import random
from dataset import CLASSES, DATA_ROOT_RAW, DATA_ROOT_PROCESSED
...
class ProcessKnots(VisionDataset):
    ...
    def __init__(self, class_name, lighting, looseness, class_idx, transform=None):
        ...
        super().__init__(DATA_ROOT_RAW, transforms=None, transform=transform)

        path = os.path.join(DATA_ROOT_RAW, '10Knots', class_name, lighting, looseness)
...
def process_images(img_size):
    ...
    dir_list = os.listdir(DATA_ROOT_PROCESSED)
    ...
    train_path = os.path.join(DATA_ROOT_PROCESSED, img_dir, 'train')
    test_path = os.path.join(DATA_ROOT_PROCESSED, img_dir, 'test')
    val_path = os.path.join(DATA_ROOT_PROCESSED, img_dir, 'val')
            ...
            for looseness in LOOSENESS_LIST: 
                ...
                for curr_split, curr_indices in zip(['train','test','val'],[train_indices,test_indices,val_indices]):
                    for idx in curr_indices:
                        ...
                        img_path = os.path.join(DATA_ROOT_PROCESSED, img_dir, curr_split, class_name, img_data.get_filename(idx))
