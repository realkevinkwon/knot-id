from dataset import CLASSES, DATA_ROOT
import os
import math
import random
...
class ProcessKnots(VisionDataset):
    ...
    def __init__(self, class_name, lighting, looseness, class_idx, transform=None):
        ...
        path = os.path.join(DATA_ROOT, '10Knots', class_name, lighting, looseness)
...
def process_images(img_size):
    ...
    train_path = os.path.join(DATA_ROOT, img_dir, 'train')
    test_path = os.path.join(DATA_ROOT, img_dir, 'test')
    val_path = os.path.join(DATA_ROOT, img_dir, 'val')
            ...
            for looseness in LOOSENESS_LIST: 
                ...
                for curr_split, curr_indices in zip(['train','test','val'],[train_indices,test_indices,val_indices]):
                    for idx in curr_indices:
                        ...
                        img_path = os.path.join(DATA_ROOT, img_dir, curr_split, class_name, img_data.get_filename(idx))
